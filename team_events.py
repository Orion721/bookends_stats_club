...
class Team_Events:
    def __init__(self):
        ...
        self.spirit = openpyxl.load_workbook(r'C:\Users\hunte\Desktop\bookends_stats_club\Club_Spirit_Scores.xlsx', data_only=True)
    ...
    def spirit_scores(self):
        worksheet = self.spirit['Sheet1']
        spirit_df = pd.DataFrame(worksheet.values)
        teams_by_event = [spirit_df[0][x - y: x] for x, y in zip(accumulate(self.event_lengths()), self.event_lengths())]
        spirit_by_event = [spirit_df[1][x - y: x] for x, y in zip(accumulate(self.event_lengths()), self.event_lengths())]
        spirit_scores_by_event = [dict(zip(teams, spirit)) for teams, spirit in zip(teams_by_event, spirit_by_event)]
        full_spirit_scores = [0 if x is None else x for x in [spirit.get(team) for teams, spirit in zip(self.teams, spirit_scores_by_event) for team in teams]]
        return full_spirit_scores

    def team_events_df(self):
        columns = {'seed': self.full_seeds(), 
                   'result': self.results(), 
                   'seed_diff': self.seed_diff(),
                   'spirit_score': self.spirit_scores(),
